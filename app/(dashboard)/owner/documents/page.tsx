...
import { ownerDocuments } from "@/lib/mock/owner"

const TYPES = ["All", "Offering", "Diligence", "Legal", "Reporting"] as const

export default function OwnerDocumentsPage() {
	const [type, setType] = useState<typeof TYPES[number]>("All")
	const [query, setQuery] = useState("")

	const filtered = ownerDocuments.filter((d) => {
	...
	return (
		<div className="px-4 lg:px-8 py-6 lg:py-10 space-y-6">
			<PageHeader
				title="Documents"
				description="Offering memos, legal agreements, diligence reports, investor reports."
				actions={
			...
			<Card padding="none">
				...
				{filtered.length === 0 ? (
					<EmptyState title="No documents" description="Upload a file to get started." icon={<FileText className="size-5" />} />
				) : (
					<ul>
						{filtered.map((d) => (
