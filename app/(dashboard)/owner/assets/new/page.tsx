				...
				<Card>
					{step === 1 && (
						<>
							...
							<div className="grid sm:grid-cols-2 gap-4">
								<TextField label="Asset name" placeholder="e.g. KL Tower Office REIT V" />
								<SelectField label="Category" options={["Real Estate", "Private Credit", "Treasuries", "Infrastructure", "Commodities"]} />
								<SelectField label="Region" options={["Singapore", "Malaysia", "Philippines", "Indonesia", "Vietnam", "United States", "Other"]} />
								<TextField label="ISIN / identifier" placeholder="Optional" />
								<TextAreaField label="Short description" placeholder="One paragraph for investor cards" />
							</div>
						</>
					)}
					...
					{step === 2 && (
						<>
							...
							<div className="grid sm:grid-cols-2 gap-4">
								<TextField label="Target raise (USD)" placeholder="6,000,000" />
								<TextField label="Minimum investment (USD)" placeholder="10" />
								<TextField label="Target APY (%)" placeholder="9.1" />
								<SelectField label="Distribution frequency" options={["Monthly", "Quarterly", "Semi-annual", "Annual", "At maturity"]} />
								<TextField label="Tenor" placeholder="Open-ended or 5y" />
								<SelectField label="Settlement asset" options={["USDC", "USDT", "EURC"]} />
							</div>
						</>
					)}
						...
						<>
							...
							<div className="rounded-lg border border-border/60 bg-card/40 p-5">
								<div className="flex items-start gap-3">
									...
									<div>
										<div className="text-sm font-medium">Pre-flight passed</div>
										<div className="text-xs text-muted-foreground mt-0.5">All required fields are filled. Submitting will lock the draft and queue it for review.</div>
									</div>
								</div>
							</div>
						</>
					...
					<div className="mt-8 flex items-center justify-between">
						...
						{step < STEPS.length ? (
							...
							<button className="h-9 px-4 rounded-md bg-primary text-primary-foreground text-xs font-medium hover:opacity-90 inline-flex items-center gap-1.5">
								Submit for review <Check className="size-3.5" />
							</button>
						)}
					</div>
				</Card>
...
function TextField({ label, placeholder }: { label: string; placeholder?: string }) {
	return (
		<label className="flex flex-col gap-1.5">
			...
			<input type="text" placeholder={placeholder} className="h-9 px-3 text-sm bg-card/60 border border-border/40 rounded-md outline-none focus:border-foreground/30" />
		</label>
	)
}
...
function SelectField({ label, options }: { label: string; options: string[] }) {
	return (
		<label className="flex flex-col gap-1.5">
			<span className="text-[10px] font-mono uppercase tracking-wider text-muted-foreground/60">{label}</span>
			<select className="h-9 px-3 text-sm bg-card/60 border border-border/40 rounded-md outline-none focus:border-foreground/30">
...
function TextAreaField({ label, placeholder }: { label: string; placeholder?: string }) {
	return (
		<label className="flex flex-col gap-1.5 sm:col-span-2">
			<span className="text-[10px] font-mono uppercase tracking-wider text-muted-foreground/60">{label}</span>
			<textarea rows={3} placeholder={placeholder} className="px-3 py-2 text-sm bg-card/60 border border-border/40 rounded-md outline-none focus:border-foreground/30 resize-y" />
		</label>
	)
}
